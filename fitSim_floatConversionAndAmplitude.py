#This version of the fitting code loops over values of different resolution parameters in discrete steps,
#fits the ampltitude of the simulation PDF and energy conversion factor, and returns the NLL value.
#This method requires binned background and simulation PDFs, and the user has two binning choices:
#"fixed" or "variable".
#
#With fixed binning, a pre-scaling factor is needed to bring the simulation to a similar range as data
# so extremely fine binning isn't necessary to capture the important details. This code assumes the guess
#will be within 50% of the true range.
#
#Variable binning does require this factor, and instead is set up to use different bins in different
#energy ranges, if the important details of the simulation and data spectrum are on different scales.
#
#In both cases, care should be taken to check the effect of binning on the best fit results.
#
...
binningType="fixed" #"fixed" or "variable", setting details in Binning section
...
#Specify bins over different ranges
	...
	#Add bins from 1000 to upperEnergyBound in steps of 100
	for i in range(1000,upperEnergyBound,100):
		binning.addBoundary(i)
...
totalSteps = numAlphaSteps*numBetaSteps*numGammaSteps
...
for testAlpha in numpy.linspace(alphaMin,alphaMax,numAlphaSteps):
	for testBeta in numpy.linspace(betaMin,betaMax,numBetaSteps):
		for testGamma in numpy.linspace(gammaMin,gammaMax,numGammaSteps):
					...
					print("Alpha="+str(testAlpha)+", beta="+str(testBeta)+", gamma="+str(testGamma))
					...
					for entry in range(0,nSimEntries):
						...
						if binningType=="fixed":
							...
							smearedVal = numpy.random.normal(mean,sigma,1)
					...
					######################################################
					##Make smearedSimDataSet a RooHistPdf (With scaling)##
					######################################################
					smearedSimDataHist = smearedSimDataSet.binnedClone()
					simPdf = ROOT.RooHistPdf("simPdf","simPdf",ROOT.RooArgList(scaling),ROOT.RooArgList(energyVar),smearedSimDataHist,1)
