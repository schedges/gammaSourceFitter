...
#fits the ampltitude of the simulation PDF, and returns the NLL value. This utilizes a RooKeysPDF for
#the bgnd and simulation PDF, which eliminates the need to use binned histograms and thus eliminates
#having to understand the effect of binning. The downside is that the RooKeysPDF could be biasing the
#result, but checking your KeysPDF matches your other PDFs in the fit range is probably sufficient.
#
...
#Set to 1 to plot RooKeysPDfs, useful initially when adjusting keys parameters
plotKeysPDfs=0
...
#If simulation a lot of events, may want to limit the number feeding into the
#simulation RooKeysPDF since that will make each fit take a longer amount of time.
#Choose number of max entries here
maxSimKeysEntries=50000
...
#NOTE: you may want to adjust the mirroring option and interpolation value to make sure your RooKeysPDF
#matches your data reasonably well
bgndKeysPdf = ROOT.RooKeysPdf("bgndKeysPdf","bgndKeysPdf",energyVar,bgndDataSet,ROOT.RooKeysPdf.NoMirror,0.85)
print("Done!")

#For checking bgnd data set and RooKeysPDF agree
if plotKeysPDfs==1:
	c2 = ROOT.TCanvas("c2","Press cntl+c while canvas selected to continue")
	bgndFrame = energyVar.frame(ROOT.RooFit.Title("Bgnd data and RooKeysPDF"))
	bgndDataSet.plotOn(bgndFrame,ROOT.RooFit.Name("BgndData"),ROOT.RooFit.Binning(200))
	bgndKeysPdf.plotOn(bgndFrame,ROOT.RooFit.Name("BgndKeysPdf"))
	bgndFrame.Draw()
	#Make legend (after Drawing frame!)
	bgndLeg = ROOT.TLegend(0.65,0.65,0.95,0.92);
	bgndDataObj = bgndFrame.findObject("BgndData");
	bgndKeysPdfObj = bgndFrame.findObject("BgndKeysPdf");
	bgndLeg.AddEntry(bgndDataObj, "Bgnd data","P");
	bgndLeg.AddEntry(bgndKeysPdfObj,"Bgnd keys PDF","L");
	bgndLeg.Draw("same")
	#Update plots
	c2.Modified()
	c2.Update()
	#Makes stuff actually plot
		...
		for testGamma in numpy.linspace(gammaMin,gammaMax,numGammaSteps):
			for testSlope in numpy.linspace(slopeMin,slopeMax,numSlopeSteps):
				for testOffset in numpy.linspace(offsetMin,offsetMax,numOffsetSteps):
					...
					#If there's more than maxSimKeysEntries, only load first maxSimKeysEntries to speed things up
					nSimEntries=simDataSet.numEntries()
					if nSimEntries > maxSimKeysEntries:
						nSimEntries=maxSimKeysEntries
					...
					#Now convert the smeared data set into a smeared RooKeysPDF
					#NOTE: You should check this does a good job approximating your simulation data set at least initially,
					#and may want to change the mirroring and interpolation values accordingly
					simPdf = ROOT.RooKeysPdf("simPdf","simPdf",energyVar,smearedSimDataSet,ROOT.RooKeysPdf.NoMirror,0.9)

					#For checking shape
					if plotKeysPDfs==1:
						c3 = ROOT.TCanvas("c3","Press cntl+c with canvas selected to continue")
						simFrame = energyVar.frame(ROOT.RooFit.Title("Sim data and keys PDF"))
						smearedSimDataSet.plotOn(simFrame,ROOT.RooFit.Name("SmearedSimDataSet"),ROOT.RooFit.Binning(200))
						simPdf.plotOn(simFrame,ROOT.RooFit.Name("SmearedSimKeysPDF"))
						simFrame.Draw()
						#Make legend (after Drawing frame!)
						simLeg = ROOT.TLegend(0.65,0.65,0.95,0.92);
						simDataObj = simFrame.findObject("SmearedSimDataSet");
						simKeysObj = simFrame.findObject("SmearedSimKeysPDF");
						simLeg.AddEntry(simKeysObj, "Sim data","P");
						simLeg.AddEntry(simKeysObj,"Sim keys PDF","L");
						simLeg.Draw("same")
						#Update plots
						c3.Modified()
						c3.Update()
					...
					##Make Model##
					##############
					model = ROOT.RooAddPdf("model","model",ROOT.RooArgList(bgndKeysPdf,simPdf),ROOT.RooArgList(scaledBgndEntriesVar,sourceCountsVar))
					...
					frame = energyVar.frame(fitRangeMin,fitRangeMax,200)
					...
					model.plotOn(frame,ROOT.RooFit.Name("Bgnd"),ROOT.RooFit.Components("bgndKeysPdf"),ROOT.RooFit.LineStyle(ROOT.kDashed), ROOT.RooFit.LineColor(ROOT.kBlack),ROOT.RooFit.FillColor(0),ROOT.RooFit.ProjWData(sourceDataSet))
